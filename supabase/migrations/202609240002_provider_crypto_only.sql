-- Replace the legacy provider enum with a crypto-only enum.
do $$
begin
  if not exists (select 1 from pg_type where typname='provider_name_crypto_only') then
    create type public.provider_name_crypto_only as enum ('crypto');
  end if;
end $$;
...
alter table public.orders alter column provider type public.provider_name_crypto_only
  using (case when provider is null then null else provider::text::public.provider_name_crypto_only end);
alter table public.payments alter column provider type public.provider_name_crypto_only
  using provider::text::public.provider_name_crypto_only;
alter table public.webhook_events alter column provider type public.provider_name_crypto_only
  using provider::text::public.provider_name_crypto_only;

alter type public.provider_name rename to provider_name_legacy;
alter type public.provider_name_crypto_only rename to provider_name;
drop type public.provider_name_legacy;

alter table public.orders add constraint orders_crypto_provider_only check (provider is null or provider = 'crypto'::public.provider_name);
alter table public.payments add constraint payments_crypto_provider_only check (provider = 'crypto'::public.provider_name);
alter table public.webhook_events add constraint webhook_events_crypto_provider_only check (provider = 'crypto'::public.provider_name);
...
begin
  ...
  if p_network not in ('bitcoin','ethereum','solana','bsc','robinhood_chain') then raise exception 'unsupported payment network'; end if;
